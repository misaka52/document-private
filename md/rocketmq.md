...
consumeFromWhere属性指定，以下几种

- CONSUME_FROM_LAST_OFFSET，一个新订阅组第一次启动从队列末尾开始消费，再次启动从上次最后消费的位置开始继续消费
- CONSUME_FROM_FIRST_OFFSET，一个新订阅组第一次启动从队列开头开始消费，再次启动从上次最后消费的位置开始继续消费
- CONSUME_FROM_TIMESTAMP，一个新订阅组第一次启动从指定时间进行消费，再次启动从上次最后消费的位置开始继续消费。具体时间consumeTimestamp指定

该参数指定第一次启动的消费者组有用，再次启动是接着上次消费的地址继续消费的。
