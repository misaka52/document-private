...
当虚拟机使用CAS获取锁时，若获取成功则将标记位记为 00；若获取失败，表示至少存在一个线程与当前线程竞争该锁，则发生锁膨胀，将锁标记位改为 10

##### 10.3.5 偏向锁

jdk6引入的锁优化措施，目的是为了消除在无竞争情况下的同步原语，提高程序的运行性能。偏向锁会偏向第一个获得它的线程。通过参数-XX:UseBiasedLocking控制偏向锁开关，默认开启
...
-  非stable参数
  ...
  - java -XX:+PrintFlagsInit	ial 查看虚拟机所有非stable参数默认值
