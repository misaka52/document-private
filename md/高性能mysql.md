...
隔离级别-SERIALIZABLE（可串行化）：通过强制控制事务串行执行，指的是数据库只能同时执行一个事务吗？不是，至少对每个操作都加锁，不关读还是写
...
TIMESAMP只是用DATETIME一半的存储空间，但范围较小，精确到秒
...
7. 子查询优化
8. 提前终止查询 
9. 列表 IN()的比较。mysql会 先对in()内的元素进行排序，然后通过二分法进行查询，有O(n)变成log(n)。可理解成一个元素对in()内n个元素对比，现在只要用二分法对比
