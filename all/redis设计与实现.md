...
**SDS的空间分配策略杜绝了内存缓存区溢出的情况**，内存不足会先扩展SDS的内空间至修改所需的要求

**字符串内存重新分配**

1. **空间预分配**：用于优化SDS的增长操作，当字符串修改后小于1M时，不仅会分配给字符串所需要的空间，还会分配与长度len相同的free空间，总的字符串实际大小=1M+1M+1byte（字符串结尾）；当SDS长度大于1M时，会额外分配1M的free空间。下次修改时，若free空间够用则不必扩容
...
- 从头结点开始，寻找下一节点分值小于等于目标节点的最大值，向前遍历。遍历路径即为最短路径
...
LPOP key #移除并返回表头元素
LRANGE start end #返回[start,end]区间的元素，0-第一个，-1表示最后一个，-2表示导数第二个元素
```
...
redis是内存数据库，提供RDB持久化功能，防止数据意外丢失。RDB持久化将数据库状态转化为RDB文件，也可以将文件还原成数据库状态
...
RDB文件的载入是服务器启动时自动执行的，没有专门的载入指令
...
文件事件处理器有四部分组成：套接字、I/O多路复用程序、文件事件分派器、事件处理器

I/O多路复用，复用一个线程监控多个套接字，并向文件事件分派器传送那些产生了事件的套接字。I/O复用程序将所有产生了事件的套接字放在一个队列中，保证有序、同步、每次一个套接字传送
