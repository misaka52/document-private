...
> 生成的代理类是一个继承自java.lang.reflect.Proxy且实现了目标接口的final类：com.sun.proxy.$Proxy${num} (${num}表示全局代理类的序号，从0开始)
...
由于JDK动态代理只能代理实现了接口的类，CGLIB可以代理普通类，通过继承类的形式生成目标类的子类。采用ASM字节码生成框架，修改字节码技术生成代理类。但不能代理final方法。
...
- CGLIB是重写父类的方法，所有不能代理final类，以及类的final方法和private 方法
...
2. spring 5.x 依然默认使用JDK代理
3. springboot1.x 默认使用JDK代理
4. springboot2.x 默认使用CGLIB代理

> spring中当默认采用JDK代理，对应实现了接口的类，采用JDK代理；否则采用CGLIB代理
>
> spring中当默认采用CGLIB代理，全部使用CGLIB代理

##### 1. 为何springboot2.x使用哪种代理

在引入了spring-boot-starter-aop包后，配置spring.aop.proxy-target-class=true，强制使用cglib代理，false表示默认使用jdk代理
...
##### 2. springboot2.x如何使用JDK代理

application配置文件中配置：spring.aop.proxy-target-class=false，配置默认使用JDK代理。

因为自动配置包中存在类AopAutoConfiguration类，如下。当配置文件中不存在属性spring.aop.proxy-target-class时默认使用CGlib代理。只要项目中有一个的类(必须是spring容器管理的bean)配置了@EnableAspectJAutoProxy(proxyTargetClass = true)，则项目中就默认使用CGLIB代理
...
##### 3. spring中如何强制使用CGLIB代理

1. 添加CGLIB库（aspectjrt-xxx.jar、aspectjweaver-xxx.jar、cglib-nodep-xxx.jar）
2. spring配置文件中加入<aop:aspectj-autoproxy proxy-target-class="true"/>
3. 强制使用cglib，spring配置文件中加入<aop:aspectj-autoproxy proxy-target-class="true"/>
...
```java
package com.ysc.learning.baseservice.designmode;
...
abstract class Observe {
    ...
    public void update(int state) {
        System.out.println(this.getClass().getSimpleName() + " update state " + state);
        this.state = state;
    }
}

class Subject {
    public List<Observe> observeList = new CopyOnWriteArrayList<>();

    private int state;

    public void setState(int state) {
        this.state = state;
        notifyAllObserve(state);
    }

    private void notifyAllObserve(int state) {
        for (Observe observe : observeList) {
            observe.update(state);
        }
    }

    public void listen(Observe observe) {
        observeList.add(observe);
    }
}

class BinaryObserve extends Observe {
...
class DecimalObserve extends Observe {

    public DecimalObserve(Subject subject) {
        this.subject = subject;
        subject.listen(this);
    }
}
