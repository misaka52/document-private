...
- 由强大的控制台界面，可创建topic、查询topic消息积压、查询单个消息
- 支持消息大量堆积
...
producer首先与任意一个NameServer建立长连接，获取Topic路由信息。然后与向Topic提供服务的Master节点建立连接，且定时发送心跳消息，发送消息

consumer首先与任意一个NameServer建立长连接，获取Topic路由信息。然后与向Topic提供服务的Master、Slave节点建立连接，并定时发送心跳消息。消费者在向Master拉取消息，Master会根据拉取偏移量与最大偏移量的距离差，和从服务器是否可读等因素建议下一次拉取走Master还是Slave
...
同一个消费者组下，通过clientId来标记生产者，负载均衡，一个生茶可订阅多个消息队列，一个消息队列只可以被一个生产者订阅。消息者消费消息队列类似于生产者
...
3. 选择消息队列：若开启broker故障延迟机制（默认未开启），选择一个正常的broker返回，且会对消息发送过长的broker开启一定的禁用期；若未开启，则轮询选择一个broker，自动避开上一个broker。
...
- CommitLog文件：保存二进制mq消息，包含mq消息+空白字符补位。以消息偏移量命名，长度20，前位补零。消息到达CommitLog文件后，ReputMessageService线程会将消息转发到到ConsumeQueue和Index文件。每条消息前4字节存储消息大小，紧接着存储消息内容
...
6. 将内存数据刷新到磁盘。同步刷盘直接保存到磁盘；异步刷盘则唤醒后台刷盘线程
7. 启动HA主从同步，仅当SYNC_MASTER模式下才会同步slave

#### 4.3 MapperFile

mapperFile类对应一个ConsumeQueue文件。重要属性如下
...
### 5. 消息消费
> 在一个JVM中，所有生产者、消费者都持有一个MQClientInstance，MQClientInstance只会启动一次
...
#### 5.5 消息消费重试
1. 消费者自动订阅了改组的重试消息主题(%RETRY%+consumerGroup)
...
## 疑问
1. CommitLog怎么存储消息？怎么查找消息？消息是否都用空白补位？顺序保存所有消息，前四字节为commitLog消息大小。可以通过offset查询。commitLog只有再最后不足保存一个消息时，才会使用空白补位
...
6. 批量消费时，若消费一定数据后中断，能否是否提交，不在消费已消费的数据？
