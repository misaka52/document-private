...
10. MsgOffsetId，MsgQuiqueId？
   1. MsgUniqueId：producer端生成的id，保存在消息属性的UNIQ_KEY中，由ip+进程号+classLoader.hashCode +距离当月一号的时间戳（4B）+计数器自增数量（2B）。保存消息时，将uniqueId作为key保存到索引文件中，其中11-14字节表示距离当月一号的时间戳，取出时间范围和key，从索引文件中查询消息，无法查询到一个月前的消息。通过控制台查询时，得到的Message ID就是值这个属性，根据msgId查询时，必须选择对应的topic，后续每个重试消息的uniqueId都相同。
