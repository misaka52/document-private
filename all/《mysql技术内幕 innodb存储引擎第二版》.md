...
带给存储引擎数据页的可靠性。当写入部分数据时机器宕机，导致部分写失效。当用重做日志进行恢复时，重做日志记录的是物理操作，只记录了修改数据，若页本身已经损坏（原未修改的数据丢失），再重做没有意义。

所以在重做日志前，用户需要一个页的副本，在写入失效时，先通过页的副本还原该页，再进行重做，这就是doublewrite
...
- sync_binlog=0，每次事务提交都刷新到binlog文件，但不刷新到磁盘数据文件
