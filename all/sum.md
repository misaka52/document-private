...
7. 机器CPU飙升问题排查
   1. 通过top定位cpu占用高的进程
   2. 使用top -Hp pid查看进程下线程资源占用情况，找出占用cpu资源高的线程id（top -Hp无法获取线程id）
   3. 通过jstack查看堆栈信息，分析死循环、死锁情况等。stack -l <进程id>  > jstack.log
   4. 查看gc情况，可通过jstat查看或查看gc日志
8. 待解决

1. 热榜功能怎么设计？
   1. top k
      1. 设计一个小顶堆，堆大小为k。对于每条热搜都保存了搜索次数，每次排名都取出热搜中的每条数据，判断其与小顶堆堆顶元素比较，若小于小顶堆则跳过该元素；若大于则移除小顶堆元素，替换成新元素，重新调整堆。此时最坏时间复杂度为nlogk
