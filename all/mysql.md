...
##### 2.3.3 using filesort优化

https://juejin.cn/post/6844903439760097294

**1.避免filesort**

减少查询字段，为其他排序字段和查询字段建立组合索引，通过索引完成排序

**2.filesort优化**

filesort的实现算法有两种

1. 双路排序。首先获取排序字段和对应的rowid，在排序完成后，再根据rowid进行回表查询，此时可能产生大量的随机访问IO
2. 单路排序。获取所有的排序字段和需要查询的字段，在内存中进行排序，无需回表

mysql通过判断对比查询结果的所有字段类型大小总和与max_length_for_sort_data字段对比，若字段总和更小则使用单路排序。否则使用双路排序

优化手段

- 调大max_length_for_sort_data值：尽量命中单路排序
- 调大sort_buffer_size：排序时都在sort buffer中排序的，若数据量大于buffer则会进行分段排序，最后合并。为避免一些分段排序及数据交换，可调大sort buffer
- 调大read_rnd_buffer_size：当进行双路排序第二次回表查询时，通过read_rnd_buffer 保存随机访问的数据，然后进行join等操作

##### 2.3.4 字段尽量不为null

1. null字段占跟更多的空间，比如索引字段，若可控多增加一个字节
2. null列在使用索引，索引统计和值计算变得更加复杂，比如金额相加
3. 使用的方便性：count(列名)会排除null字段行，不等号、not in都会排除null行记录
4. 在myisam中会导致固定大小的索引变为可变大小的索引
...
从服务器若是备份的作用，就可以关闭一些实时的配置，如sync_binlog=0等。增加从服务器也可以分散读，降低读的压力
...
1. 主库压力大，产生了阻塞，导致binlog日志同步的进行阻塞。可能因为数据库的并发更新量大，或者DDL语句的产生，导致大部分数据发生改变
2. 前期同步master情况下，延迟高
...
1. 采用分库策略或增加分库的数量，分散数据库压力
2. 单个库读写分析，主写从读，从配置多个，减少主库压力
3. 服务层增加缓存或缓存优化，减少数据库的访问量
4. 不同业务的数据库隔离，放在不同实例上，避免相互响应

**硬件方面**

1. 升级配置。cpu增加，内存增加，更换ssd
2. 网络带宽升级

**主从配置**

1. sync_binlog在slave端设置0，有主线程定时持久化二进制日志
...
按照hash取模分片规则分库分表时，当数据库表过少需进行扩容时，迁移步骤如下

1. 成倍扩容，表数量保证为2的幂次方。下面加入扩展为原来的两倍
2. 新库表通过监听原库binlog日志，一个库数据分别保存一份到两个新库中，等待同步基本完成
3. 停止应用程序，等待一段时间保证主从同步全部完成（若表中不存在自增主键，可以先部署应用，等最后同步差不多再切断主从关系）
4. 更换程序新数据源，启动应用程序（停止一些全表路由的查询）
5. 根据分片规则，删除各库表冗余数据（开启全表路由的查询）

#### 3 双写迁移

1. 开启双写功能，一份数据同时写新老两个库，要实现分布式事务
2. 通过程序将老库全量数据同步到新库中，等待同步完成
3. 校验新老库数据量是否相等，若相等则进入下一步。不相等再等待校验
4. 更换分库分表规则
5. 清理新老两库冗余数据
...
1. 如何在线修改一个大数据量的表结构，比如增加字段，或添加索引
   1. 直接修改会导致锁表，不推荐
   2. 在线修改-短暂停服
      1. 首先创建目标新表，开启binlog日志同步，等待同步的差不多了进入下一步
      2. 停服，等待同步完成
      3. 新老表名变更，数据源信息配置变更，启动服务
   3. 在线修改-不停服
      1. 首先创建目标新表，开启binlog日志同步，等待同步的差不多了进入下一步
      2. 更换新表配置启动服务。insert操作新表，select&update-新表>老表，delete-新表&老表
      3. 等待同步完成，关闭假双数据源配置，只操作新表
