...
1. Using where：未命中索引，只能在mysql server层过滤
2. Using index：仅通过扫描索引便可完成sql。比如命中覆盖索引
3. using temporary：表示使用临时表来存储结果，常用于排序和分组查询
4. using filesort：无法利用索引完成的排序称为“文件排序”
5. using join buffer：该值强调了在连表时没有使用索引，并且需要缓冲区来保存中间结果。出现该情况时需要注意添加索引
6. impossible where：表示使用了无效的where查询条件，无法查询到结果。比如where 1 = 0
7. select tables optimized away：该值意味着仅通过使用索引，优化器可能仅从聚合函数结果中返回一行
8. using index condition：表示使用了索引下推ICP（mysql5.7版本之后支持）
